...
impl View for VisualizerView {
    ...
    fn draw(&self, cx: &mut DrawContext, canvas: &mut Canvas) {
        ...
        let line_width = cx.style.dpi_factor as f32 * 1.0;
...
impl View for VisualizerViewAlt {
    fn element(&self) -> Option<&'static str> {
        Some("visualizer")
    }
    ...
    fn draw(&self, cx: &mut DrawContext, canvas: &mut Canvas) {
        ...
        for i in 0..BUFFER_SIZE {
            ...
            path.move_to(
                x,
                bounds.y + (bounds.h * self.visualizer.get(i).clamp(0.0, 1.0)),
            );
            path.line_to(x, bounds.y);
        }
        canvas.stroke_path(&mut path, &paint);
    }
}
